...
class Operaciones extends Conectar
{
    public function listar_preguntas($fecha_inicio, $fecha_fin, $vehi_id)
    {
        ...
        GROUP BY pre_vehiculo
            ...
            WHERE preoperacional.pre_fecha_crea_form BETWEEN ? AND ? and preoperacional.pre_vehiculo =?
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(30, 5, '06 Julio 2020', 0, 1, 'L');

        $this->SetX(150);
        $this->Cell(30, 5, 'Codigo:', 0, 0, 'R');
        $this->Cell(30, 5, 'ME-F-20', 0, 1, 'L');
        ...
        $this->SetY(15);
        $this->SetX(70);
        $this->Cell(60, 10, 'Formato Preoperacional De Bomba Estacionaria', 0, 0, 'C');
        $this->Ln(25);
    }
...
if (!empty($agrupado_por_formulario)) {
    ...
    foreach ($agrupado_por_formulario as $formulario => $operaciones) {
        foreach ($operaciones as $operacion) {
            ...
            $fecha_poliza = isset($operaciones[0]['vehi_poliza_vence']) ? $operaciones[0]['vehi_poliza_vence'] : '01/01/1999';
            ...
            if ($Fecha != $ultima_fecha) {
                ...
                $pdf->Ln(5);
                $pdf->Cell(60, 5, 'SOAT:' . '   ' . date_format(new DateTime($soat), 'd/m/Y'), 1, 0, 'L', 0);
                $pdf->Cell(40, 5, 'Serie:' . 'N/A', 1, 0, 'L', 0);
                $pdf->Cell(40, 5, 'Capaciad:' . 'N/A', 1, 0, 'L', 0);
                ...
                $pdf->Cell(32, 20, 'Item', 1, 0, 'C', 1);
                $pdf->Cell(108, 20, 'Elemento', 1, 0, 'C', 1);
                ...
                $pdf->Ln(10);
                $pdf->SetFont('Arial', '', 9);
                $pdf->Cell(32, 116, 'Bomba Estacionaria', 1, 0, 'C', 1);
                ...
                foreach ($operaciones as $operacion) {
                    ...
                    $pdf->SetX(42);
                    $pdf->Cell(108, 4, utf8_decode($operacion['suboper_nombre']), 1, 0, 'L', 0);
                    $pdf->SetX(150);

                    if ($operacion['pre_repuesta'] == 'B') {
                        $pdf->Cell(17, 4, 'X', 1, 0, 'C');
                        $pdf->Cell(17, 4, '', 1, 0, 'C');
                        $pdf->Cell(16, 4, '', 1, 0, 'C');
