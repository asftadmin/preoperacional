...
class Operaciones extends Conectar
{
    public function listar_preguntas($fecha_inicio, $fecha_fin,$vehi_id)
    {
        ...
        vehiculos.vehi_modelo, preoperacional.pre_estado, preoperacional.pre_formulario
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->SetY(15);
        $this->SetX(70);
        $this->Cell(60, 10, 'Inspeccion De Fresadora', 0, 0, 'C');
        $this->Ln(25);
    }
...
if (!empty($agrupado_por_formulario)) {
    ...
    foreach ($agrupado_por_formulario as $formulario => $operaciones) {
        foreach ($operaciones as $operacion) {
            ...
            $Fecha = isset($operacion['pre_fecha_crea_form']) ? $operacion['pre_fecha_crea_form'] : 'N/A';
            $horometraje = isset($operacion['pre_kilometraje_inicial']) ? $operacion['pre_kilometraje_inicial'] : 'N/A';
            $marca = isset($operacion['vehi_marca']) ? $operacion['vehi_marca'] : 'N/A';
            $modelo = isset($operacion['vehi_modelo']) ? $operacion['vehi_modelo'] : 'N/A';
            $observaciones = isset($operacion['pre_observaciones']) ? $operacion['pre_observaciones'] : 'N/A';
            $operario = isset($operacion['conductor_nombre_completo']) ? $operacion['conductor_nombre_completo'] : 'N/A';
            $estado = isset($operacion['pre_estado']) ? $operacion['pre_estado'] : 'N/A';
            ...
            if ($Fecha != $ultima_fecha) {
                ...
                $pdf->Ln(5);
                $pdf->Cell(60, 10, 'Inspeccionado por:', 0, 0, 'R');
                $pdf->Cell(0, 10, '', 1, 0, 'C');
                $pdf->Ln();
                $pdf->Cell(60, 10, 'Reportado:', 0, 0, 'R');
