...
var getURLParameter = function(sParam) {
  ...
  const ES_TARIFA_FIJA = new Set([71,72,9,10,43,15,45,44,67,54,52,60,50,90,64,65]);
 ...
 $(document).on('input', '.viajes-input, .km-inicial-input, .km-final-input, .tarifa-input', function () {
    const $curTr  = $(this).closest('tr');
    const $parent = $curTr.hasClass('child') ? $curTr.prev('tr') : $curTr;
    const $child  = $parent.next('tr').hasClass('child') ? $parent.next('tr') : $();

    const rowApi  = tabla.row($parent);
    if (!rowApi.node()) return;

    const data   = rowApi.data();
    const actId  = parseInt(data[9])   || 0;
    const vol    = parseFloat(data[2]) || 0;

    const findVal = (selector) => {
        const $input = $parent.add($child).find(selector);
        if (!$input.length) return 0;
        const v = parseFloat(($input.val() || '').toString().replace(',', '.'));
        return isNaN(v) ? 0 : v;
    };
    ...
    console.log('actId:', actId, 'vol:', vol, 'viajes:', viajes, 'kmin:', kmin, 'kfin:', kfin, 'tarifa:', tarifa);

    const rangoKm  = Math.max(0, kfin - kmin);
    const subtotal = calcularSubtotal(actId, vol, viajes, kmin, kfin, tarifa);

    console.log('rangoKm:', rangoKm, 'subtotal:', subtotal);

    const paint = ($tr) => {
        if (!$tr || !$tr.length) return;
        $tr.find('.rango-km').text(rangoKm.toFixed(2));
